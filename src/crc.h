...
{
	cm_t ubt_cm;
	volatile crc32;//最终输出的crc32值
	volatile p_cm_t spi_crc32;
	
	spi_crc32 = &ubt_cm;
	memset(spi_crc32, 0, sizeof(cm_t));
	
	ubt_crc32_init(spi_crc32);
	ubt_crc32_pack(spi_crc32, &buf[0], buf_len);
	ubt_crc32_pack(spi_crc32, &buf[0], buf_len);
	ubt_crc32_pack(spi_crc32, &buf[0], buf_len);
	......
	crc32 = ubt_crc32_pack_end(spi_crc32, 0, 0); or  crc32 = (spi_crc32, &buf[0], buf_len);
}
*/
#ifdef  __cplusplus
}
